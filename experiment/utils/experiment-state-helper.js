...
// here check if stakeholder already joined or openec channel on BC
...
/*
    1 - OPEN (HERE CHECK STAKEHOLDER ON BLOCKCHAIN)
    2 - JOIN (HERE CHECK STAKEHOLDER ON BLOCKCHAIN)
    3 - OFF CHAIN TX
    4 - CLOSE
*/
async function get_last_channel_state(channelid) {
    ...
    try {
        ...
        all_relevant_states.forEach((curr) => {
            ...
            } else if (curr.action === 2) { // join - samo en state
                const address = curr.state[0].address;
                if (state_data.hasOwnProperty(address)) {
                    state_data[address] += curr.state[0].balance;
                } else {
...
async function get_state_channel_history(channel_id, cid) {
    ...
    try {
        if (channel_id != null) {
            ...
            // console.log(ipfs_state_encoded);
        } else {
